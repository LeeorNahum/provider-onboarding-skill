...
A lean skill that fills a web product's environment and configures its providers after the repo already exists. The repo's current code and committed `.env.example` files are the spec. This skill just gets every value into the right store and wires each dashboard and CLI.
